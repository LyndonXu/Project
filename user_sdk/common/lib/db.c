...
typedef struct
{
	int32_t idxfd; /* fd for index file */
	int32_t datfd; /* fd for data file */
 ...
 * Internal functions.
 */
static DB *_db_alloc(int32_t);
static void _db_dodelete(DB *);
static int32_t _db_find_and_lock(DB *, const char *, int32_t);
static int32_t _db_findfree(DB *, int32_t, int32_t);
static void _db_free(DB *);
static DBHASH _db_hash(DB *, const char *);
static char *_db_readdat(DB *);
static off_t _db_readidx(DB *, off_t);
static off_t _db_readptr(DB *, off_t);
static void _db_writedat(DB *, const char *, off_t, int32_t);
static void _db_writeidx(DB *, const char *, off_t, int32_t, off_t);
...
DBHANDLE db_open(const char *pathname, int32_t oflag, ...)
{
	...
	if (oflag & O_CREAT)
	{
		...
		{
			int32_t oflag_tmp = oflag;
			oflag_tmp |= O_EXCL;
			db->idxfd = open(db->name, oflag_tmp, mode);
			strcpy(db->name + len, ".dat");
			db->datfd = open(db->name, oflag_tmp, mode);
 ...
 */
static DB *_db_alloc(int32_t namelen)
{
	DB *db;
...
void db_destroy(DBHANDLE h)
{
	char *pTmp;
	PRINT("remove file: %s\n", ((DB *) h)->name);
	remove(((DB *) h)->name);
	pTmp = strrchr(((DB *) h)->name, '.');
	if (pTmp != NULL)
	{
		strcpy(pTmp, ".idx");
		PRINT("remove file: %s\n", ((DB *) h)->name);
 ...
 */
static int32_t _db_find_and_lock(DB *db, const char *key, int32_t writelock)
{
 ...
 */
static void _db_writedat(DB *db, const char *data, off_t offset, int32_t whence)
{
	struct iovec iov[2];
 ...
 */
static void _db_writeidx(DB *db, const char *key, off_t offset, int32_t whence,
		off_t ptrval)
{
	struct iovec iov[2];
 ...
 */
static int32_t _db_findfree(DB *db, int32_t keylen, int32_t datlen)
{
	int32_t rc;
